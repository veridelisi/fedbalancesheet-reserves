...
def comparison():
    ...
    fig.add_trace(go.Scatter(x=df["Time"], y=df["Loans"], mode="lines",
                             name="Loans", line=dict(width=3, color="#0A0A0A",dash="dot")))
    add_shading(fig); yaxis_k(fig)
...
with tEA:
    ...
    with area_tabs[4]:
        ...
        COLORS = ["#0e0d0d", "#0e0d0dcd", "#0e0d0ddc", "#0e0d0d93"]
...
with tEC:
    ...
    # ====================== TAB 3: Loans (CROSS-BORDER + LOCAL) ======================
    with tabLoans:
        ...
        sel_loans = st.multiselect(
            ...
            key="loans_cb_country_select"   # benzersiz key
        )
